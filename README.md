...
| `SEERRSENSE_TRUSTED_PROXY_HOPS` | How many proxy hops in front of the pod to trust when deriving the client address. Default 1. Setting it higher than the number of real proxies lets a caller forge `X-Forwarded-For` and get a fresh bucket from every per-IP limit |
